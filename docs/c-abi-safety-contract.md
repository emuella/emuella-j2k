...
Status: accepted design for the first C ABI implementation. No C ABI symbols,
headers, libraries or compatibility promise exist yet.
...
and unsafe-code rules that must govern the first implementation. It deliberately
does not fix function names, complete C structure layouts or build artefacts.
Those become public compatibility commitments only when an implementation and
generated header pass the gates below.
