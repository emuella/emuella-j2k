...
static EmuellaJ2kStatus read_at(void *, std::uint64_t offset,
                                std::uint8_t *destination, std::size_t length) noexcept {
  try {
    if (offset > sizeof(CODESTREAM) || length > sizeof(CODESTREAM) - static_cast<std::size_t>(offset)) {
      return 98;
    }
    std::memcpy(destination, CODESTREAM + static_cast<std::size_t>(offset), length);
...
int main() {
  EmuellaJ2kSourceV0 source{sizeof(source), EMUELLA_J2K_ABI_VERSION, 0,
                            sizeof(CODESTREAM), nullptr, read_at};
